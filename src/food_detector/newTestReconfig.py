...
class TestReconfigModule(TestRepubModule):
    ...
    def spanet_callback(self, img):
        ...
        if yaml_node['push_direction'] is None:
            ...
            score = self.fake_scores(yaml_node['push_direction'])
            # score = 0.9
        ...
        mark = Marker(header=img.header, text=yaml.dump(info_map), mesh_resource=yaml.dump(info_map))
        # print(img.header)
        ma.markers.append(mark)

        print(ma.markers[0].mesh_resource)
    ...
    def fake_scores(self, push_direction):
        scores_dict = {'left':0.9, 'up':0.6, 'right':0.5, 'down':0.8}
