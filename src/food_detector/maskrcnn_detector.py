...
from .util import load_maskrcnn, load_label_map, load_maskrcnn_label_map
from . import ada_feeding_demo_config as conf


class MaskRCNNDetector(PoseEstimator, CameraSubscriber, ImagePublisher):
    def __init__(
            self,
            use_cuda,
            node_name=rospy.get_name(),
            maskrcnn_checkpoint=conf.maskrcnn_checkpoint,
            label_map_file=conf.maskrcnn_label_map,
            camera_tilt=1e-5,
        ...
        import os
        print(os.getcwd(), maskrcnn_checkpoint)    # RUN FROM BASE OF FOODRECOGNITION_WS
        self.maskrcnn = load_maskrcnn(use_cuda, maskrcnn_checkpoint)
        self.label_map = load_maskrcnn_label_map(label_map_file)
    ...
    def detect_objects(self):
        ...
        bgr_img = np.array(img)[:,:,::-1] / 255.
        x = torch.tensor(bgr_img, dtype=torch.float32).permute((2, 0, 1)).unsqueeze(0)    # reshape to (1, C, H, W)

        # MaskRCNN Prediction
        with torch.no_grad():
            if self.use_cuda:
                outputs = self.maskrcnn(x.cuda())
            else:
                outputs = self.maskrcnn(x)

            all_boxes = outputs[0]['boxes'].cpu()
            all_labels = outputs[0]['labels'].cpu()
            all_scores = outputs[0]['scores'].cpu()

            valid_idx = torch.argwhere(all_scores > conf.maskrcnn_thresh).squeeze(-1)

            boxes = all_boxes[valid_idx]
            labels = all_labels[valid_idx]
